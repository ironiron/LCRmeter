...
TEST_CASE( "sends data 8 bit")
{
  ...
  REQUIRE(testing::send_data.size()==2);
  REQUIRE(testing::send_data[0]==addr);
  REQUIRE(testing::send_data[1]==data);
}
...
TEST_CASE( "sends data 16 bit")
{
  ...
  REQUIRE(er==I2C::ErrorCode::OK);
  REQUIRE(testing::send_data.size()==3);
  REQUIRE(testing::send_data[0]==addr);
  REQUIRE(testing::send_data[1]==0x32);
  REQUIRE(testing::send_data[2]==0x56);
}
...
TEST_CASE( "sends data 16 bit + address 8 bit")
{
  ...
  REQUIRE(er==I2C::ErrorCode::OK);
  REQUIRE(testing::send_data.size()==4);
  REQUIRE(testing::send_data[0]==addr);
  REQUIRE(testing::send_data[1]==mem_addr);
  REQUIRE(testing::send_data[2]==0x32);
  REQUIRE(testing::send_data[3]==0x56);
}
...
TEST_CASE( "sends data 16 bit + address 8 bit continuously")
{
  ...
  REQUIRE(er==I2C::ErrorCode::OK);
  REQUIRE(testing::send_data.size()==8);
  REQUIRE(testing::send_data[0]==addr);
  REQUIRE(testing::send_data[1]==mem_addr);
  REQUIRE(testing::send_data[2]==0x32);
  REQUIRE(testing::send_data[3]==0x56);
  REQUIRE(testing::send_data[4]==0x04);
  REQUIRE(testing::send_data[5]==0x04);
  REQUIRE(testing::send_data[6]==0x55);
  REQUIRE(testing::send_data[7]==0x89);
}


TEST_CASE( "return error when timeout occurs")
{
  testing::status_addr_bit=0;//set hardware bit which indicates that addres bit is not sent
  uint8_t addr=0x24;
  uint8_t data=0x45;
  testing::send_data.clear();
  ...
  testing::status_addr_bit=1;//clean
}

TEST_CASE( "return error when NACK occur")
{
  testing::status_nack_bit=1;//NACK occured, and no addr bit is set
  testing::status_addr_bit=0;
  uint8_t addr=0x24;
  uint8_t data=0x45;
  testing::send_data.clear();
  ...
  testing::status_addr_bit=1;//clean
  testing::status_nack_bit=0;
}

TEST_CASE( "return error when bus is busy")
{
  testing::status_TxE_bit=0;//bus_busy bit set while Transfer buffer not empty
  testing::status_bus_busy_bit=1;
  uint8_t addr=0x24;
  uint8_t data=0x45;
  testing::send_data.clear();
  ...
  testing::status_TxE_bit=1;//clean
  testing::status_bus_busy_bit=0;
}
...
TEST_CASE( "Send data circular thru DMA")
{
  ...
  REQUIRE(er==I2C::ErrorCode::OK);
  REQUIRE(testing::send_data.size()==8);
  REQUIRE(testing::send_data[7]==addr);
  REQUIRE(testing::send_data[0]==mem_addr);
  REQUIRE(testing::send_data[1]==0x32);
  REQUIRE(testing::send_data[2]==0x56);
  REQUIRE(testing::send_data[3]==0x04);
  REQUIRE(testing::send_data[4]==0x04);
