...
private:
  ...
  uint16_t * make_write(uint16_t *a,uint32_t lenght)//for continous write
  {
    for (;lenght>0;)
      {
	lenght--;
...
template<bool use_MCP4725>
  ...
  if (use_MCP4725 !=0 )
    {
      conn.Send_Data(address_write,make_write(value));
    }
  else
    {
...
template<bool use_MCP4725>//not sure how isit supposed to work
//TODO check with real hardware
  ...
  if (use_MCP4725 !=0 )
    {
      //conn.Send_Data_Circular(address_write,make_write(value,lenght), lenght);
    }
  else
    {
      auto temp=make_write(static_cast<uint8_t>(MCP47FEB_addresses::DAC0_volatile));
